...
filterRating = totalsRating.filter(lambda x: x[1][1] >= 100)
# print(filterRating.collect())

averagesRating = filterRating.mapValues(lambda x: x[0] / x[1])
# (영화 번호, 평점 평균)
...
# (평점 평균, 영화 번호) 평점 높은 순에서 낮은 순으로 sort
